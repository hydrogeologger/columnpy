...
result = pd.read_csv('rainfall.csv')

for i in tb_pandas.result_df.keys():
    print(i)
    result = pd.concat([result, pd.read_csv(i+'.csv')[str(i)]],axis=1)

result.to_csv('wwl_weather_rawdata.csv')
...
file_path = current_path+'/wwl_weather_rawdata.csv'
...
if os.path.isfile(file_path):
    ...
    for Cleanup in glob.glob(current_path+'/*.csv*'):
        ...
        if not Cleanup.endswith('wwl_weather_rawdata.csv'):
            os.remove(Cleanup)
