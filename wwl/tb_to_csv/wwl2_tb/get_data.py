...
writer = pd.ExcelWriter('wwl2_rawdata_V2.xlsx', engine='xlsxwriter') #If there is no module named "xlsxwriter", just sudo pip install xlsxwriter.
j=0
k=0
for i in tb_pandas.result_df.keys():
    ...
    if len(tb_pandas.result_df[i])>100:
        ...
        chart.add_series({
            ...
            #'values':     [i, 1, 1, 1950, 1],
            'categories': [i, 1, 0, 33110, 0],
            'values':     [i, 1, 1, 33110, 1],
