...
for nt=2:2:length(nod)-1
    ...
    %% -------------  sub 1 Groundwater table over time at sa2  --------------
    a.sub1=subplot('position'...
         ,[fig_pos.left,fig_pos.bottom,...
          fig_pos.length,fig_pos.height]);       
    a.plot1=plot(time_measured_data_day(1:1000:end),...
            Tbl.sa2_groundwater_table_rise_mm(1:1000:end)/1000,'b.');hold on
    a.plot1=plot(time_nod_day(1:nt),watertable_diff_sa2_m(1:nt),...
             'r-','linewidth',a.lw);
    ylabel({'groundwater table';'elevation (m)'},'FontSize',a.fs);
    legend({'sa2','model'},'Location','northwest');    
    ylim([-1 8]);
    xlim([0 140]);
    set(gca,'XTick',[])
    %% -------------  sub 1-1 Groundwater table over time outside the basin  --------------
    a.sub11=subplot('position'...
     ,[fig_pos.left+0.5,fig_pos.bottom,...
      fig_pos.length,fig_pos.height]);       
    a.plot11=plot(time_measured_data_day(1:1000:end),...
            Tbl.sa1_groundwater_table_rise_mm(1:1000:end)/1000,'b.');hold on
    a.plot11=plot(time_measured_data_day(1:1000:end),...
            Tbl.sa4_groundwater_table_rise_mm(1:1000:end)/1000,'g.');        
    a.plot11=plot(time_nod_day(1:nt),watertable_diff_sa1_m(1:nt),...
             'r-','linewidth',a.lw);
    ylim([-1 1]);
    xlim([0 140]);
    set(gca,'XTick',[])
    legend({'sa1','sa4','model'},'Location','southeast');
    ylabel({'groundwater table';'elevation (m)'},'FontSize',a.fs);   
     %% -------------  sub 2 surface water table over time  --------------
    a.sub2=subplot('position'...
         ,[fig_pos.left,fig_pos.bottom-fig_pos.height,...
          fig_pos.length,fig_pos.height]); 
    [M,I] = min(abs(time_nod_day(nt)-time_measured_data_day))  
    a.plot2=plot(time_measured_data_day(1:I),...
            (Tbl.p3_cs451(1:I)-0.2213)*1000,'b.');hold on   
    xlim([0 140]);
    ylim([0 1000]);
    set(gca,'XTick',[])
...
%     %legend('show','Location','East')
    ...
    a.plot3=plot(time_nod_day(1:nt),...
    ...
    %% -------------  sub 4 conc over time  --------------
    a.sub4=subplot('position'...
         ,[fig_pos.left,fig_pos.bottom-2*fig_pos.height,...
          fig_pos.length,fig_pos.height]);
    a.plot4=plot(time_nod_day(1:nt),...
    ...
    %% -------------  sub 5 pressure over time  --------------
    
    salt_dep=arrayfun(@(y) y.terms{sm_idx}(inp.nn1),nod(1:nt))/2165/area1*1000;
    a.sub5=subplot('position'...
         ,[fig_pos.left+0.5,fig_pos.bottom-2*fig_pos.height,...
          fig_pos.length,fig_pos.height]);
    a.plot5=plot(time_nod_day(1:nt),...
    ...
    a.sub6=subplot('position'...
         ,[fig_pos.left,fig_pos.bottom-3*fig_pos.height-0.07,...
    ...
    a.sub7=subplot('position'...
         ,[fig_pos.left+0.5,...
         fig_pos.bottom-3*fig_pos.height-0.07,...
          fig_pos.length+0.02,fig_pos.height]);
    ...
    a.sub8=subplot('position'...
         ,[fig_pos.left,...
         fig_pos.bottom-4*fig_pos.height-0.25,...
