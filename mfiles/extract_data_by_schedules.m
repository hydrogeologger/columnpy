...
%    % for dt85g. raw here means the direct extraction of data from datalogger.
    ...
    sched(i).raw.dt85g.vwc_pt1  =dt85g.vwc_pt1(dt85g_mask);  % top of the arcylic 
    sched(i).raw.dt85g.vwc_pt2  =dt85g.vwc_pt2(dt85g_mask);  % sensor at the bottom
    sched(i).raw.dt85g.vwc_pt3  =dt85g.vwc_pt3(dt85g_mask);
    sched(i).raw.dt85g.vwc_pt4  =dt85g.vwc_pt4(dt85g_mask);  % in the consolidometer
    sched(i).raw.dt85g.vwc_pt5  =dt85g.vwc_pt5(dt85g_mask);  % dead sensor
    sched(i).raw.dt85g.vwc_pt6  =dt85g.vwc_pt6(dt85g_mask);  % middle of the arcylic
    % for em50
...
%        0.8,sched(i).time_day_ay2);
    ...
    sched(i).dt85g.vwc_pt1 =ec5(1).alpha*csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt1,0.8,sched(i).time_day_ay)+ec5(1).beta;
    sched(i).dt85g.vwc_pt2 =ec5(2).alpha*csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt2,0.8,sched(i).time_day_ay)+ec5(2).beta;
    sched(i).dt85g.vwc_pt3 =ec5(3).alpha*csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt3,0.8,sched(i).time_day_ay)+ec5(3).beta;
    sched(i).dt85g.vwc_pt4 =ec5(4).alpha*csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt4,0.8,sched(i).time_day_ay)+ec5(4).beta;
    sched(i).dt85g.vwc_pt5 =ec5(5).alpha*csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt5,0.8,sched(i).time_day_ay)+ec5(5).beta;
    sched(i).dt85g.vwc_pt6 =ec5(6).alpha*csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt6,0.8,sched(i).time_day_ay)+ec5(6).beta;


    sched(i).dt85g.vwc_pt1 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt1,0.8,sched(i).time_day_ay).^ec5(1).gama...
	-ec5(1).zero_sat_rs^ec5(1).gama)/ (ec5(1).full_sat_rs^ec5(1).gama -ec5(1).zero_sat_rs^ec5(1).gama);
    sched(i).dt85g.vwc_pt2 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt2,0.8,sched(i).time_day_ay).^ec5(2).gama...
	-ec5(2).zero_sat_rs^ec5(2).gama)/ (ec5(2).full_sat_rs^ec5(2).gama -ec5(2).zero_sat_rs^ec5(2).gama);
    sched(i).dt85g.vwc_pt3 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt3,0.8,sched(i).time_day_ay).^ec5(2).gama...
	-ec5(3).zero_sat_rs^ec5(3).gama)/ (ec5(3).full_sat_rs^ec5(3).gama -ec5(3).zero_sat_rs^ec5(3).gama);
    sched(i).dt85g.vwc_pt4 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt4,0.8,sched(i).time_day_ay).^ec5(2).gama...
	-ec5(4).zero_sat_rs^ec5(4).gama)/ (ec5(4).full_sat_rs^ec5(4).gama -ec5(4).zero_sat_rs^ec5(4).gama);
    sched(i).dt85g.vwc_pt5 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt5,0.8,sched(i).time_day_ay).^ec5(2).gama...
	-ec5(5).zero_sat_rs^ec5(5).gama)/ (ec5(5).full_sat_rs^ec5(5).gama -ec5(5).zero_sat_rs^ec5(5).gama);
    sched(i).dt85g.vwc_pt6 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
        ,sched(i).raw.dt85g.vwc_pt6,0.8,sched(i).time_day_ay).^ec5(2).gama...
	-ec5(6).zero_sat_rs^ec5(6).gama)/ (ec5(6).full_sat_rs^ec5(6).gama -ec5(6).zero_sat_rs^ec5(6).gama);


     if strcmpi(sched(i).tag,'arcylic_small_pet')
	% two problem issued when dealling with low pet:
        % significant rise of rs when the water level is just about the surface.
	% solve: choose the maxmum rs as the value that corresponding to 1
        % the minimum ec doesn't correlate
        % solve: unify all the ec values for all sensors
        spline_coef=0.8;
       	max_rs=max(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
      	                ,sched(i).raw.dt85g.vwc_pt1,spline_coef,sched(i).time_day_ay));
	sched(i).raw.dt85g.vwc_pt1(1:800)=max_rs
        sched(i).dt85g.vwc_pt1 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
                ,sched(i).raw.dt85g.vwc_pt1,spline_coef,sched(i).time_day_ay).^ec5(1).gama...
                -500^ec5(1).gama)/ (max_rs^ec5(1).gama -500^ec5(1).gama);
        max_rs=max(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
                        ,sched(i).raw.dt85g.vwc_pt2,spline_coef,sched(i).time_day_ay));
	sched(i).raw.dt85g.vwc_pt2(1:800)=max_rs
        sched(i).dt85g.vwc_pt2 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
                ,sched(i).raw.dt85g.vwc_pt2,spline_coef,sched(i).time_day_ay).^ec5(2).gama...
                -500^ec5(2).gama)/ (max_rs^ec5(2).gama -500^ec5(2).gama);
        max_rs=max(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
                        ,sched(i).raw.dt85g.vwc_pt6,spline_coef,sched(i).time_day_ay));
	sched(i).raw.dt85g.vwc_pt6(1:800)=max_rs
        sched(i).dt85g.vwc_pt6 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
                ,sched(i).raw.dt85g.vwc_pt6,spline_coef,sched(i).time_day_ay).^ec5(2).gama...
                -500^ec5(6).gama)/ (max_rs^ec5(6).gama -500^ec5(6).gama);
			
%        max_rs=max(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%                        ,sched(i).raw.dt85g.vwc_pt1,spline_coef,sched(i).time_day_ay));
%       sched(i).dt85g.vwc_pt1 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%	        ,sched(i).raw.dt85g.vwc_pt1,spline_coef,sched(i).time_day_ay).^ec5(1).gama...
%		-ec5(1).zero_sat_rs^ec5(1).gama)/ (max_rs^ec5(1).gama -ec5(1).zero_sat_rs^ec5(1).gama);
%       	max_rs=max(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%      	                ,sched(i).raw.dt85g.vwc_pt2,spline_coef,sched(i).time_day_ay));
%        sched(i).dt85g.vwc_pt2 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%	        ,sched(i).raw.dt85g.vwc_pt2,spline_coef,sched(i).time_day_ay).^ec5(2).gama...
%		-ec5(2).zero_sat_rs^ec5(2).gama)/ (max_rs^ec5(2).gama -ec5(2).zero_sat_rs^ec5(2).gama);
%       	max_rs=max(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%      	                ,sched(i).raw.dt85g.vwc_pt6,spline_coef,sched(i).time_day_ay));
%        sched(i).dt85g.vwc_pt6 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%	        ,sched(i).raw.dt85g.vwc_pt6,spline_coef,sched(i).time_day_ay).^ec5(2).gama...
%		-ec5(6).zero_sat_rs^ec5(6).gama)/ (max_rs^ec5(6).gama -ec5(6).zero_sat_rs^ec5(6).gama);

%	    sched(i).dt85g.vwc_pt1 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%	        ,sched(i).raw.dt85g.vwc_pt1,0.003,sched(i).time_day_ay).^ec5(1).gama...
%		-ec5(1).zero_sat_rs^ec5(1).gama)/ (ec5(1).full_sat_rs^ec5(1).gama -ec5(1).zero_sat_rs^ec5(1).gama);
%	    sched(i).dt85g.vwc_pt2 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%	        ,sched(i).raw.dt85g.vwc_pt2,0.003,sched(i).time_day_ay).^ec5(2).gama...
%		-ec5(2).zero_sat_rs^ec5(2).gama)/ (ec5(2).full_sat_rs^ec5(2).gama -ec5(2).zero_sat_rs^ec5(2).gama);
%	    sched(i).dt85g.vwc_pt6 =(csaps(sched(i).raw.dt85g.time_digi-sched(i).start_digi...
%	        ,sched(i).raw.dt85g.vwc_pt6,0.003,sched(i).time_day_ay).^ec5(2).gama...
%		-ec5(6).zero_sat_rs^ec5(6).gama)/ (ec5(6).full_sat_rs^ec5(6).gama -ec5(6).zero_sat_rs^ec5(6).gama);
     end
