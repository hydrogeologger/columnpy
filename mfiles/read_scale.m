...
scale.time_str=scale.textdata(2:size(scale.textdata,1),1);
%% (D)ate and (T)ime for (S)cale stored by (D)igits
scale.time_digi=datenum(scale.time_str,'mmm/dd/yy HH:MM:SS');
%% (N)umber of (R)eadings at (S)cale
scale.number_readings=size(scale.time_digi,1);
fprintf(1,'Read scale data complete \n');
