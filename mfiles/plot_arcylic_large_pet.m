i=find(strcmpi({sched.tag},'arcylic_large_pet'));
iv=1;
...
plot(sched(i).time_day_ay(1:iv:end),...
	sched(i).accu_evap(1:iv:end)*c.m2mm,'ro');hold on
plot( sched(i).raw.scale.time_digi(1:iv:end)-sched(i).start_digi,...
    sched(i).raw.scale.water_loss_m(1:iv:end)*c.m2mm,'go')
...
subplot(5,1,2);
plot(sched(i).time_day_ay(1:iv:end),sched(i).evap(1:iv:end)*c.ms2mmday,'bo');hold on
...
subplot(5,1,3);
plot(sched(i).dt85g.vwc_pt1,sched(i).evap*c.ms2mmday,'bo','displayname','raw');hold on
...
subplot(5,1,4);
plot(sched(i).accu_evap*c.m2mm,sched(i).evap*c.ms2mmday,'bo','displayname','raw');hold on
xlabel('Accu. evap.(-)');ylabel('evaporation rate (mm/day)')

subplot(5,1,5);
plot(sched(i).dt85g.vwc_pt1,sched(i).accu_evap*c.m2mm,'bo','displayname','raw');hold on
...
%
subplot(5,1,2);
plot(sched(i).time_day_ay(1:iv:end),sched(i).dt85g.temp_pt1(1:iv:end),...
    'ro','displayname','dt85-pt1');hold on
plot(sched(i).time_day_ay(1:iv:end),sched(i).dt85g.temp_pt2(1:iv:end),...
    'go','displayname','dt85-pt2');hold on
plot(sched(i).time_day_ay(1:iv:end),sched(i).dt85g.temp_pt3(1:iv:end),...
    'bo','displayname','dt85-pt3');hold on
plot(sched(i).time_day_ay(1:iv:end),sched(i).dt85g.temp_pt4(1:iv:end),...
