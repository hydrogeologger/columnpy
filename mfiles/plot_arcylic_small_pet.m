...
subplot(5,1,1);
plot(sched(i).time_day_ay...
	,sched(i).accu_evap*c.m2mm,'ro');hold on
plot( sched(i).raw.scale.time_digi-sched(i).start_digi...
	,sched(i).raw.scale.water_loss_m*c.m2mm,'go')
...
subplot(5,1,2);
plot(sched(i).time_day_ay,...
	sched(i).evap*c.ms2mmday,'ro');hold on
...
subplot(5,1,3);
plot(sched(i).dt85g.vwc_pt1,sched(i).evap*c.ms2mmday,'ro','displayname','raw');hold on
...
subplot(5,1,4);
plot(sched(i).accu_evap*c.m2mm,sched(i).evap*c.ms2mmday,'ro','displayname','raw');hold on
xlabel('Accu. evap.(-)');ylabel('evaporation rate (mm/day)')

subplot(5,1,5);
plot(sched(i).dt85g.vwc_pt1,sched(i).accu_evap*c.m2mm,'ro','displayname','raw');hold on
