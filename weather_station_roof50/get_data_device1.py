...
tb_pandas=thingsboard_to_pandas.tingsboard_to_pandas('tb_credential_device1.json')   # input is the location of the json file
...
#time_now_str = time.strftime("%Y/%b/%d %H:%M", time.localtime)
#start_time_str = "2022/Jan/01 00:00"
#tb_pandas.get_data(start_time=start_time_str, end_time=time_now_str) #modified library of "thingsboard_to_pandas" for automatically pulling the data from thingsboard and saving as a xlsx file with plot
...
writer = pd.ExcelWriter('weather_rawdata_UQroof_device1.xlsx', engine='xlsxwriter') #If there is no module named "xlsxwriter", just sudo pip install xlsxwriter.
j=0
k=0
