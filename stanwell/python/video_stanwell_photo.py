...
#path_im='/home/osboxes/stanwell/'
#files = filter(os.path.isfile, glob.glob(path_im + "*.jpg"))
#files_name=[i.split('/')[-1] for i in files]
#files_name.sort()
#photo_taken_time=[i[0:16] for i in files_name]
...
for ii in file_name:
    ...
    fig, ax = plt.subplots(7,2,sharex=True,figsize=(16,10))
    ...
    ax_temp.set_position([0.85,0.05,0.15,0.45])
    #
    ...
    ax[0][0].bar(ta.index[:idx_rainm], df_last['rainmm'][:idx_rainm], width=1.8,edgecolor='white',lw=0.1)
    ax[0][0].set_ylim([-0.1,33])
    ...
    ax[1][0].bar(ta.index[:idx_rainm],df_mean['pet_mmPday'][:idx_rainm],width=1.0,color='brown',edgecolor='white',label='Pote.\nevap.',lw=0.1)
    ax[1][0].bar(ta.index[:idx_rainm],df_mean['aet_mmPday'][:idx_rainm],width=1.0,color='orange',edgecolor='white',label='Actu.\nevap.',lw=0.1)
    ax[1][0].set_ylim([-0.1,11])
    ...
    ax[6][0].plot(daily_data_manual.index[:idx_settlement], daily_data_manual['settlement_mm'][:idx_settlement], '-',color='maroon',markersize=ms,markeredgewidth=mew,fillstyle='full', markeredgecolor='r',label='5 cm')
    ax[6][0].set_ylim([-1,179])
