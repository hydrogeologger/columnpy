...
data_file_path=current_path+'/data/bacteria_sali_scale/'    # warning, all the files should be .dat DO NOT FORGET THE LAST SLASH
...
data_header=['hum10','hum3','hum4','hum5','hum6','hum7','hum8','hum9','scale1','scale2','date_time','tmp10','tmp3','tmp4','tmp5','tmp6','tmp7','tmp8','tmp9']
...
data.df['hum10'].loc[data.df['hum10']<1]=np.nan;
data.df['hum7'].loc[data.df['hum7']<1]=np.nan;
data.df['hum8'].loc[data.df['hum8']<1]=np.nan;
data.df['hum9'].loc[data.df['hum9']<1]=np.nan;
data.df['tmp7'].loc[data.df['tmp7']<1]=np.nan;
data.df['tmp8'].loc[data.df['tmp8']<1]=np.nan;
data.df['tmp9'].loc[data.df['tmp9']<1]=np.nan;
data.df['tmp10'].loc[data.df['tmp10']<1]=np.nan;
